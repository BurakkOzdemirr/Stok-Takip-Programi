...
namespace proje1
{
    public partial class egzoz : Form
    {
        ...
        void vericek()
        {
            string sec = "select  eg.*,kul.kullaniciadi from egzoz as eg,kullaniciler as kul where eg.tckimlikno=kul.tckimlikno";
            OleDbDataAdapter da = new OleDbDataAdapter(sec, con);
            if (ds.Tables["egzoz"] != null)
                ds.Tables["egzoz"].Clear();
            da.Fill(ds, "egzoz");
            toolStripLabel1.Text = "Stok sayısı=" + ds.Tables["egzoz"].Rows.Count;

        }

        private void egzoz_Load(object sender, EventArgs e)
        {
            lka.Text = Form2.ka;
        ...
        private void by_Click(object sender, EventArgs e)
        {
            ...
            yenikayitmi = true;
            kacincikayit = ds.Tables["egzoz"].Rows.Count;
        ...
        private void bkaydet_Click(object sender, EventArgs e)
        {
            ...
            if (yenikayitmi)
            {
                ...
                cmd.CommandText = "insert into egzoz (stokadi,stokadedi,tckimlikno,eklemetarihi,aciklama,resim,tanefiyat) Values (@sadi,@sadedi,@tc,@tarih,@aciklama,@resim,@tf)";
                cmd.Parameters.AddWithValue("@sadi", tbsadi.Text);
                cmd.Parameters.AddWithValue("@sadedi", tbsadedi.Text);
            ...
            else
            {
                ...
                cmd.CommandText = "update egzoz set stokadi=@sa,stokadedi=@sadedi,eklemetarihi=@tarih,aciklama=@aciklama,resim=@resim,tanefiyat=@tf where stokkodu=@sk";
        ...
        private void bsil_Click(object sender, EventArgs e)
        {
            ...
            if (c == DialogResult.Yes)
            {
                OleDbCommand cmd = new OleDbCommand();
                cmd.Connection = con;
                cmd.CommandText = "delete from egzoz where stokkodu=@sk";
                cmd.Parameters.AddWithValue("@sk", int.Parse(tbsk.Text));
                cmd.ExecuteNonQuery();
                vericek();
        ...
        {
            geri.Enabled = true;
            baş = ds.Tables["bodykit"].Rows.Count;
            bs.Position++;
            if (--baş == bs.Position)
                ileri.Enabled = false;
        }
        ...
        {
            bkaydet.Visible = biptal.Visible = false; tbekleyenka.ReadOnly = false; vericek();
        }
        ...
        {
            string sec = "select * from egzoz where stokadi like '%" + saarama.Text + "%'";
            OleDbDataAdapter da = new OleDbDataAdapter(sec, con);
            if (ds.Tables["egzoz"] != null)
                ds.Tables["egzoz"].Clear();
            da.Fill(ds, "egzoz");
        ...
        private void cbka_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string sec = "select  eg.*,kul.kullaniciadi from egzoz as eg,kullaniciler as kul where eg.tckimlikno=kul.tckimlikno and eg.tckimlikno='" + cbka.SelectedValue + "'";
                OleDbDataAdapter da = new OleDbDataAdapter(sec, con);
                if (ds.Tables["egzoz"] != null)
                    ds.Tables["egzoz"].Clear();
                da.Fill(ds, "egzoz");
